...
Where-Object {$_.StartName -eq “tcbmn\administrator”} | # Return only services who’s “Log On As” property is set to domain\administrator
...
$ComputerList = Get-ADComputer -Filter 'Name -NotLike "WFH*" -and Name -NotLike "KM*" -and Name -notlike "V*" ' 
$i = 0
...
ForEach($Computer in $ComputerList)
{
    if(Test-Connection -Count 1 -ComputerName $Computer.Name -Quiet)
    {
        ...
        if(Get-wmiobject -ComputerName $Computer.Name Win32_Service | Where-Object {$_.StartName -eq “tcbmn\administrator”} -ErrorAction SilentlyContinue -OutVariable obj )
        {
            #$obj = Get-wmiobject -ComputerName $Computer.Name Win32_Service | Where-Object {$_.StartName -eq “tcbmn\administrator”} -ErrorAction SilentlyContinue |select Name,StartMode,State
           foreach($o in $obj)
           {
